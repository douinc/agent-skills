...
사용자가 GitHub 이슈 링크와 함께 디자인 작업을 시작하거나, 해당 기능의 UI/UX 방향·구현 방법을 요청하면 링크된 이슈를 부모 이슈로 취급하고 `dou-uxui-issues` 스킬을 함께 사용한다.
...
`UI/UX Done`은 디자인 작업 완료를 뜻하며 개발 핸드오프나 개발 준비 완료를 뜻하지 않는다. 이슈 생성·수정·관계 설정·프로젝트 상태 변경의 상세 절차와 등록 전 확인 게이트는 `dou-uxui-issues`를 따른다.
