...
작업 시작 전 CLAUDE.md의 `product_design_workflow` 키를 확인한다. 없으면 민수님께 프로젝트 경로와 설정을 물어본다.
...
- **파일 없음** → 지금까지 구현된 화면을 기반으로 초기 `docs/ia.md` 생성을 민수님께 제안한다
...
민수님과 대화로 아래 항목을 채운 뒤, 정리해서 컨펌을 받는다.
...
확실하지 않은 항목은 민수님께 반드시 물어본다. 스펙이 모호하면 설계가 흔들린다.
...
**작업 시작 전 반드시 읽을 것:**
- `DESIGN.md` — 디자인 철학, 색상 규칙, 컴포넌트 사용 규칙, Do/Don't
- `app/globals.css` — 색상 토큰, radius 값
- `app/design-system/sections/` — 사용 가능한 컴포넌트 목록
...
컨펌 후 React 컴포넌트를 작성한다.

**와이어프레임 vs 구현 — 핵심 원칙:**
와이어프레임은 레이아웃·구조·플로우를 확정하는 도구다. 시각적 스타일의 기준이 아니다.
구현할 때는 와이어프레임의 UI 스타일을 그대로 따르지 않는다.

| 우선순위 | 방법 |
|---------|------|
| 1 | `components/ui/` 또는 `components/[제품명]/`에 있는 컴포넌트를 그대로 사용 |
| 2 | 없으면 디자인 느낌이 유사한 컴포넌트로 대체 |
| 3 | 대체 불가능하면 새 컴포넌트 만들어 `components/[제품명]/`에 등록 |

**비주얼 디자인 방식:**
별도 Figma 디자인 없이 `DESIGN.md` 토큰 + shadcn 컴포넌트 기반으로 구현한다. 로컬 미리보기(⑧)에서 시각적 피드백을 받아 반영한다.

**규칙:**
- 모든 색상은 CSS 토큰 사용 (`bg-primary`, `text-muted-foreground` 등) — 하드코딩 금지
- 컴포넌트는 `components/ui/`의 shadcn 컴포넌트 우선
- **컴포넌트는 기본 스타일 그대로 사용한다 — `className`으로 임의 덮어쓰기 금지.** 스타일 변경이 필요하면 민수님께 먼저 물어본다.
- 빈 상태 UI 반드시 구현
- 반응형 구현 — 모바일·데스크탑 모두
...
오류가 있으면 민수님께 전달하기 전에 모두 수정한다.

---

## ⑧ 로컬 미리보기

`localhost:3000/[페이지경로]` 에서 민수님이 직접 확인.
